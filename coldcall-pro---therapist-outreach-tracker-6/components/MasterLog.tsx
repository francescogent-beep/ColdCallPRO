          ...
          return (
            ...
            >
               <div className="flex justify-between items-start mb-3">
                ...
                <div className="flex gap-1.5 ml-2">
                  ...
                  <button 
                    ...
                    className="p-2.5 bg-slate-100 text-slate-400 rounded-full border border-slate-200"
                  ...
                  <button 
                    ...
                    className={`p-2.5 rounded-full border transition-colors ${hardNoPromptId === entry.id ? 'bg-rose-600 text-white border-rose-600' : 'bg-rose-50 text-rose-600 border-rose-100'}`}
                    title="Quick Hard No"
                  >
              ...
              {hardNoPromptId === entry.id && (
                <div className="mb-3 p-3 bg-rose-50 rounded-xl border border-rose-100 animate-in fade-in slide-in-from-top-2">
                  <div className="text-[10px] font-black text-rose-600 uppercase tracking-widest mb-2">Who answered?</div>
                  <div className="flex gap-2">
                    <button onClick={() => handleQuickHardNo(entry, WhoAnswered.OWNER)} className="flex-1 py-2 bg-rose-600 text-white rounded-lg text-[9px] font-black uppercase tracking-widest">Owner</button>
                    <button onClick={() => handleQuickHardNo(entry, WhoAnswered.GATEKEEPER)} className="flex-1 py-2 bg-white text-rose-600 border border-rose-200 rounded-lg text-[9px] font-black uppercase tracking-widest">Gatekeeper</button>
                  </div>
                </div>
              )}
                ...
                <tr key={entry.id} className={`hover:bg-slate-50 group transition-colors ${isCalledToday ? 'bg-slate-100/60' : ''}`}>
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex justify-end gap-2 items-center">
                      {hardNoPromptId === entry.id ? (
                        <div className="flex items-center gap-1 bg-rose-50 p-1 rounded-lg border border-rose-100 animate-in fade-in zoom-in-95">
                          <span className="text-[8px] font-black text-rose-600 uppercase px-1">Who?</span>
                          <button onClick={() => handleQuickHardNo(entry, WhoAnswered.OWNER)} className="px-2 py-1 bg-rose-600 text-white rounded text-[8px] font-black uppercase">Owner</button>
                          <button onClick={() => handleQuickHardNo(entry, WhoAnswered.GATEKEEPER)} className="px-2 py-1 bg-white text-rose-600 border border-rose-200 rounded text-[8px] font-black uppercase">GK</button>
                          <button onClick={() => setHardNoPromptId(null)} className="p-1 text-rose-300 hover:text-rose-600"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
                        </div>
                      ) : (
                        <>
                          <button 
