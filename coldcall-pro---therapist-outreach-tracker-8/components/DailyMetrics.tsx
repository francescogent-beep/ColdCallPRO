...
import { CallLogEntry, WhoAnswered, WhatsAppSent, CallOutcome, SessionMetric } from '../types';
import { OUTCOME_OPTIONS } from '../constants';

interface DailyMetricsProps {
  entries: CallLogEntry[];
  historicalSessions: SessionMetric[];
}

type MetricView = 'daily' | 'weekly' | 'monthly' | 'custom' | 'all-time';

const DailyMetrics: React.FC<DailyMetricsProps> = ({ entries, historicalSessions }) => {
  ...
  const filteredEntries = useMemo(() => {
    ...
    switch (view) {
      ...
      case 'all-time': {
        return entries.filter(e => !!e.lastCallDate);
      }
      default:
        return [];
    }
  ...
  const filteredSessions = useMemo(() => {
    const getSessionsInRange = (start: Date, end: Date) => {
      const endOfDay = new Date(end);
      endOfDay.setHours(23, 59, 59, 999);
      return historicalSessions.filter(s => {
    ...
    switch (view) {
      case 'daily': {
        const start = new Date(todayStr);
        return getSessionsInRange(start, start);
      }
      case 'weekly': {
        const start = new Date();
        start.setDate(today.getDate() - 7);
        return getSessionsInRange(start, today);
      }
      case 'monthly': {
        const start = new Date();
        start.setDate(today.getDate() - 30);
      ...
      case 'custom': {
        if (!customStart || !customEnd) return [];
        return getSessionsInRange(new Date(customStart), new Date(customEnd));
      }
      case 'all-time':
  ...
  }, [historicalSessions, view, customStart, customEnd, todayStr]);

  const calc = useMemo(() => {
    // Combine session metrics with entry metrics for more accurate volume
    const sessionTotalCalls = filteredSessions.reduce((acc, s) => acc + s.calls, 0);
    const sessionTotalRejections = filteredSessions.reduce((acc, s) => acc + s.rejections, 0);

    const total = Math.max(filteredEntries.length, sessionTotalCalls);
    ...
    const rejections = Math.max(filteredEntries.filter(e => [
      CallOutcome.NOT_INTERESTED_HARD,
      CallOutcome.NOT_NOW,
      CallOutcome.ALREADY_GOT_SOMEONE
    ].includes(e.outcome)).length, sessionTotalRejections);
  ...
  const stats = [
    ...
    { label: 'Rejection %', value: `${calc.rejectionRate}% (${calc.rejections})`, sub: 'Total Rejections', color: 'bg-rose-400' },
  ...
  const exportData = (format: 'json' | 'csv') => {
    ...
    if (format === 'json') {
      ...
      a.href = url;
      a.download = `coldcall_pro_export_${new Date().toISOString().split('T')[0]}.json`;
      a.click();
    } else {
      // CSV Export
      let csv = 'Type,ID,Business/Start,Contact/End,Phone/Calls,Profession/Rejections,City,Status,Outcome,Date\n';
      
      // Leads
      entries.forEach(e => {
        csv += `Lead,${e.id},"${e.businessName}","${e.contactName}",${e.phone},${e.profession},${e.city},${e.leadStage},${e.outcome},${e.lastCallDate || ''}\n`;
      });
      
      // Sessions
      historicalSessions.forEach(s => {
        csv += `Session,${s.id},${new Date(s.startTime).toISOString()},${s.endTime ? new Date(s.endTime).toISOString() : ''},${s.calls},${s.rejections},,,, \n`;
      });
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `coldcall_pro_export_${new Date().toISOString().split('T')[0]}.csv`;
      a.click();
    }
  };
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (data.leads || data.sessions) {
          if (window.confirm('Import data? This will merge with your existing cloud data.')) {
            // We trigger the bulk add in App.tsx via a custom event or we'd need a callback
            // For now, we'll just alert that they should use the Master Log import for leads
            // and we'll implement a proper callback if needed.
            alert('Please use the Import button in the Prospects tab to import leads. Session import is coming soon.');
          }
        }
      } catch (err) {
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end border-b border-slate-200 pb-6 gap-4">
        <div>
          ...
          <div className="flex items-center gap-4 mt-1">
            <p className="text-slate-400 text-[11px] font-black uppercase tracking-widest">Analytics & Data Management</p>
            <div className="flex gap-2">
              <button onClick={() => exportData('json')} className="text-[9px] font-black text-indigo-600 uppercase hover:underline">Export JSON</button>
              <button onClick={() => exportData('csv')} className="text-[9px] font-black text-indigo-600 uppercase hover:underline">Export CSV</button>
            </div>
          </div>
        </div>
        
        <div className="flex bg-slate-200/50 p-1 rounded-xl text-[10px] font-black uppercase overflow-x-auto max-w-full">
          {(['daily', 'weekly', 'monthly', 'all-time', 'custom'] as MetricView[]).map((v) => (
            <button
              key={v}
              onClick={() => setView(v)}
              className={`px-4 py-1.5 rounded-lg transition-all whitespace-nowrap ${
      ...
      <div className="bg-slate-900 rounded-[2rem] p-8 text-white relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
          ...
          <div className="space-y-1">
            <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Rejection Rate</div>
            <div className="text-2xl font-black">{calc.rejectionRate}%</div>
            <div className="h-1 bg-slate-800 rounded-full overflow-hidden">
              <div className="h-full bg-rose-400" style={{ width: `${calc.rejectionRate}%` }}></div>
            </div>
          </div>
          <div className="space-y-1">
                    ...
                    <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                      <div 
                        className={`h-full rounded-full transition-all duration-500 ${
                          outcome === CallOutcome.BOOKED ? 'bg-rose-600' :
                          outcome === CallOutcome.INTERESTED ? 'bg-indigo-500' :
                          [CallOutcome.NOT_INTERESTED_HARD, CallOutcome.NOT_NOW, CallOutcome.ALREADY_GOT_SOMEONE].includes(outcome as CallOutcome) ? 'bg-rose-400' :
