...
const DailyMetrics: React.FC<DailyMetricsProps> = ({ entries }) => {
  ...
  const handleExportMetrics = () => {
    if (filteredEntries.length === 0) {
      alert('No metric data to export for this range.');
      return;
    }

    const headers = ['Category', 'Value'];
    const summaryRows = stats.map(s => [s.label, s.value]);
    
    const breakdownHeader = ['', ''];
    const breakdownLabel = ['Outcome Breakdown', 'Count'];
    const outcomeRows = Object.values(CallOutcome).map(outcome => {
    ...
    link.setAttribute('download', `metrics_${view}_${new Date().toISOString().split('T')[0]}.csv`);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end border-b border-slate-200 pb-4 gap-4">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          <button 
            ...
            className="px-3 py-1 bg-slate-100 text-slate-600 border border-slate-200 rounded text-[9px] font-black uppercase tracking-widest hover:bg-slate-200 flex items-center gap-1.5"
          >
